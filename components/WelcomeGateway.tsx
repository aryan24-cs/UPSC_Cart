...
export default function WelcomeGateway({ onBrowseAsGuest }: WelcomeGatewayProps = {}) {
  ...
  const handleDemoLogin = async (role: "USER" | "ADMIN") => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.error || "Failed to authenticate demo account.");
        setIsLoading(null);
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="min-h-[calc(100vh-4.5rem)] flex flex-col items-center justify-center px-4 py-8 sm:py-16 bg-[#FAFAF9]">
      <div className="w-full max-w-2xl text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm sm:text-base text-[#71717A] max-w-md mx-auto">
            Your hyper-local peer-to-peer marketplace for UPSC essentials in Old Rajinder Nagar, Mukherjee Nagar, and Karol Bagh.
          </p>
        </div>
        ...
        <div className="pt-2">
          ...
          <p className="text-xs text-[#71717A] mt-0.5">
            Select your role or use demo accounts to explore the platform
          </p>
        </div>
          ...
          <div className="rounded-xl border border-[#E4E4E7] bg-white p-5 sm:p-6 flex flex-col justify-between hover:border-[#D4D4D8] transition-all hover:shadow-xs">
            <div>
              ...
              <p className="text-xs text-[#71717A] mt-1.5 leading-relaxed">
                Buy & sell standard textbooks, coaching notes, study furniture, find verified rooms, and bargain with peers.
              </p>
            </div>
            ...
            <div className="pt-6 space-y-2">
              ...
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-1.5 rounded-lg border border-[#E4E4E7] bg-[#FAFAF9] px-4 py-2 text-xs font-medium text-[#18181B] hover:bg-[#F4F4F5] transition-colors disabled:opacity-50 cursor-pointer"
              >
                <span>{isLoading === "USER" ? "Entering demo..." : "Explore as Demo User"}</span>
              </button>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-[#E4E4E7] bg-white p-5 sm:p-6 flex flex-col justify-between hover:border-[#D4D4D8] transition-all hover:shadow-xs">
            ...
            <div className="pt-6 space-y-2">
              ...
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-1.5 rounded-lg border border-[#E4E4E7] bg-[#FAFAF9] px-4 py-2 text-xs font-medium text-[#18181B] hover:bg-[#F4F4F5] transition-colors disabled:opacity-50 cursor-pointer"
              >
                <span>{isLoading === "ADMIN" ? "Entering admin..." : "Explore as Demo Admin"}</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="pt-1">
          {onBrowseAsGuest ? (
            <button
              ...
              className="text-xs text-[#71717A] hover:text-[#18181B] font-medium transition-colors underline underline-offset-4 cursor-pointer"
            >
              Or browse marketplace as guest &rarr;
            </button>
          ) : (
            <Link
              href="/marketplace"
              className="text-xs text-[#71717A] hover:text-[#18181B] font-medium transition-colors underline underline-offset-4"
            >
              Or browse marketplace as guest &rarr;
            </Link>
          )}
